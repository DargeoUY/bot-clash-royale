...
async function requireRegistration(userId: number): Promise<string | null> {
  ...
  if (!player) return '⚠️ No tenés una cuenta vinculada.\nUsá /registrar #TAG primero.';
  return null;
}
...
export interface TgReply { text: string; privateText?: string; extraMessages?: string[]; }
...
export async function handleTelegramCommand(chatId: number, userId: number, text: string, isGroup: boolean): Promise<TgReply | null> {
  ...
  if (cmd === '/perfil') {
    if (!checkCooldown(userId)) return { text: '⏳ Esperá unos segundos antes de usar otro comando.' };
    const err = await requireRegistration(userId); if (err) return { text: err };
    const player = await prisma.player.findFirst({ where: { telegramId: String(userId) } });
